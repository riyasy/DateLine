...
internal sealed class OutlookHelper
{
    ...
    public Dictionary<DateTime, string> GetTaskStrings(DateTime startDate, DateTime endDate)
    {
        Logger.Info("Get Task Strings started");
        var result = new Dictionary<DateTime, string>();

        foreach (Outlook.AppointmentItem oAppt in _oCalendar.Items)
            if (oAppt.Start.Date >= startDate && oAppt.Start.Date <= endDate)
            {
                result.TryAdd(oAppt.Start.Date, "");
                var apptString = oAppt.Start.ToString("HH:mm") + "  " + oAppt.Subject + "\n";
                result[oAppt.Start.Date] += apptString;
